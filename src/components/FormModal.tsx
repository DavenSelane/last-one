...
const deleteAction = {
  ...
  grade: deleteGrade,
  user: deleteTutor, // Users can be deleted using deleteTutor (same structure)
};
...
const TutorForm = dynamic(() => import("./forms/TutorForm"), {
...
const forms: {
  ...
  tutor: (setOpen, type, data, relatedData) => (
    <TutorForm
      type={type}
      data={data}
      setOpen={setOpen}
...
const FormModal = ({
  ...
  const Form = () => {
    const [state, formAction] = useFormState(deleteAction[table], {
      success: false,
      error: false,
    });
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 bg-black bg-opacity-60 z-[9999] flex items-center justify-center">
          <div className="bg-white p-4 rounded-md w-[90%] md:w-[70%] lg:w-[60%] xl:w-[50%] 2xl:w-[40%] max-h-[60vh] overflow-auto">
            <Form />
            <div
